...
Ennemi::Ennemi(std::string nomEnnemi, sf::Sprite newSprite, float valVitesse)
{
...
bool Ennemi::deplacement(Clock & clock, bool tileBloquant[NB_TILE], int map[MAX_LARGEUR_MAP][MAX_HAUTEUR_MAP], int etape)
{
    if (m_moving && m_vivant)
    {
        ...
        if(verifColisionDeplacementEnnemi(tileBloquant, map) && etape > 2)
        {
            _sprite.move(-m_velocity * elapsed.asSeconds());
            m_moving = false;
            m_velocity.x = 0;
...
bool Ennemi::verifColisionDeplacementEnnemi(bool tileBloquant[NB_TILE], int map[MAX_LARGEUR_MAP][MAX_HAUTEUR_MAP])
{
    int nb3 = 0;
    ...
    if(m_velocity.x < -30)
    {
        if(m_velocity.y < -30)///5 haut gauche
        {
            ...
            if(tileBloquant[map[(posMob_X) / LARGEUR_TILE][(posMob_Y - nb3) / HAUTEUR_TILE]] == 1 )/*||
        ...
        {
            if(tileBloquant[map[(posMob_X) / LARGEUR_TILE][(posMob_Y + HAUTEUR_TILE + nb3) / HAUTEUR_TILE]] == 1)/* ||
                        tileBloquant[map[(posMob_X - nb3) / LARGEUR_TILE][(posMob_Y + 16) / HAUTEUR_TILE]] == 1)*/
            {
                collision = true;
        ...
        {
            if (tileBloquant[map[(posMob_X - nb3) / LARGEUR_TILE][(posMob_Y + 16) / HAUTEUR_TILE]] == 1)
            {
                collision = true;
    ...
    else if(m_velocity.x > 30)
    {
        ...
        {
            if (tileBloquant[map[(posMob_X + LARGEUR_TILE) / LARGEUR_TILE][(posMob_Y - nb3) / HAUTEUR_TILE]] == 1)/* ||
        ...
        {
            if (tileBloquant[map[(posMob_X  + LARGEUR_TILE) / LARGEUR_TILE][(posMob_Y + HAUTEUR_TILE + nb3) / HAUTEUR_TILE]] == 1)/* ||
                        tileBloquant[map[(posMob_X + LARGEUR_TILE + nb3) / LARGEUR_TILE][(posMob_Y + 16) / HAUTEUR_TILE]] == 1)*/
            {
                collision = true;
        ...
        {
            if (tileBloquant[map[(posMob_X + TAILLE_BLOC + nb3) / LARGEUR_TILE][(posMob_Y + 16) / HAUTEUR_TILE]] == 1)
            {
                collision = true;
    ...
    else
    {
        ...
        {
            if (tileBloquant[map[(posMob_X + 16) / LARGEUR_TILE][(posMob_Y - nb3) / HAUTEUR_TILE]] == 1)
            {
                collision = true;
        ...
        {
            if (tileBloquant[map[(posMob_X + 16) / LARGEUR_TILE][(posMob_Y + TAILLE_BLOC + nb3) / HAUTEUR_TILE]] == 1)
            {
                collision = true;
